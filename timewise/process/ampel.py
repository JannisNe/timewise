...
def make_ampel_job_file(cfg_path: Path, template_path: Path | None = None) -> Path:
    ...
    ampel_job = template.replace("TIMEWISE_CONFIG_PATH", str(cfg_path)).replace(
        "ORIGINAL_ID_KEY", ori_id_key
    )
